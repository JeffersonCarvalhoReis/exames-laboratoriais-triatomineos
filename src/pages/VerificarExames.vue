...
export default {
  ...
  methods: {
    ...
    async getYearsFromFirestore() {
      ...
      examsSnapshot.forEach((doc) => {
        ...
        const examYear = new Date(
          this.formattedDate(examData.dataExame)
        ).getFullYear();
    ...
    async getQuartersFromFiresotre() {
      const examsSnapshot = await getDocs(
        query(
          ...
          where("dataExame", ">=", `${this.selectedYear}-01-01`),
          where("dataExame", "<=", `${this.selectedYear}-12-31`)
        )
      );
      ...
      examsSnapshot.forEach((doc) => {
        const examData = doc.data();
        const examMonth =
          new Date(this.formattedDate(examData.dataExame)).getMonth() + 1;
    ...
    async getMonthsFromFirestore() {
      ...
      this.filteredMonths = Array.from(monthSet)
        .sort()
        .map((monthNumber) => {
          const monthObj = this.monthNames.find(
            (m) => m.number === monthNumber
          );
          return {
            value: String(monthObj.number).padStart(2, "0"),
            label: monthObj.name,
          };
        });
    },
