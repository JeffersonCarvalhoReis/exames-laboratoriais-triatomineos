<template>
  <q-page class="gerar-resultados-page">
    ...
    <div class="q-mt-md">
      <div>De {{ formatarData(intervaloDatas.start) }} até {{ formatarData(intervaloDatas.end) }}</div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        columns: [
          { name: 'details', label: '', align: 'left' }, // Coluna para a lupa
          { name: 'dataExame', label: 'Data do Exame', field: 'dataExame', align: 'left', format: val => this.formatarData(val) },
          { name: 'responsavel', label: 'Responsável', field: 'responsavel', align: 'left' },
          { name: 'actions', label: 'Exlcuir', align: 'left' } // Coluna para a lixeira
        ],
