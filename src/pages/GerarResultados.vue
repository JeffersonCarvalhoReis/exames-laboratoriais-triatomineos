<template>
  <q-page class="gerar-resultados-page">
    ...
    <div v-for="(exames, key) in quadrimestresOrdenados" :key="key" class="q-mb-md">
      ...
      <div class="q-mt-md">
        <div>
          De {{ exames.menorData }} até {{ exames.maiorData }}
        </div>
        <div>{{ key }}</div>
      </div>
...
<script>
  ...
  export default {
    ...
    async mounted() {
      ...
      console.log(this.resultadosPorQuadrimestre)
    },
    ...
    computed: {
      ...
      quadrimestresOrdenados() {
        return Object.keys(this.resultadosPorQuadrimestre)
          .sort((a, b) => {
            const ultimoResultadoA = new Date(this.resultadosPorQuadrimestre[a].maiorData);
            const ultimoResultadoB = new Date(this.resultadosPorQuadrimestre[b].maiorData);
    ...
    methods: {
      ...
      async buscarResultados() {
        ...
        querySnapshot.forEach((doc) => {
          ...
          resultadosPorQuadrimestre[key].resultados.push(doc.data());
        });
        ...
        for (const quadrimestre in resultadosPorQuadrimestre) {
          ...
          if (resultados.length > 0) {
            ...
            resultados = resultados.sort((b, a) => new Date(a.dataExame) - new Date(b.dataExame));

            const datas = resultados.map((resultado) => new Date(resultado.dataExame));
            ...
            resultadosPorQuadrimestre[quadrimestre].menorData = menorData.toLocaleDateString();
            resultadosPorQuadrimestre[quadrimestre].maiorData = maiorData.toLocaleDateString();
          }
        }
